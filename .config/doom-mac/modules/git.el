...
(after! magit
  ;; 1) Don’t treat *magit…* buffers as popups
  (set-popup-rule! "^\\*magit" :ignore t)

  ;; 2) Use Magit’s own traditional display function
  (setq +magit-display-buffer-fn #'magit-display-buffer-traditional)
  ...
  ;; (You can re-enable a full-frame view by swapping in this instead:
  ;; (setq +magit-display-buffer-fn #'magit-display-buffer-fullframe-status-v1))
  ...
  (defun endless/issue-md-link-from-id (id)
    (format "[%s](https://atlassian.net/browse/%s)" id id))
    ...
    (when (string-match "\\`\\([[:alpha:]]+\\(?:-[0-9]+\\)\\)-\\(.*\\)\\'" branch)
      (let ((prefix (upcase (match-string 1 branch)))
        ...
        (let* ((case-fold-search nil)
               ...
               (capitalized  (if (string-match "[A-Z]" spaced)
                                 spaced
                               (capitalize spaced))))
  ...
  (defun endless/visit-pull-request-url ()
    "Visit the current branch's PR on Github or GitLab."
    (interactive)
    (let* ((base-branch    (replace-regexp-in-string "^origin/" ""
  ...
  ;; Bind C-q in Magit status to pop open the PR URL
