;; Global Auto-Insert for Ruby Snippets
(after! yasnippet
  (defun kyo/auto‑insert‑ruby‑template ()
    "When visiting a brand‑new, empty Ruby file ending in `_spec.rb` or `_controller.rb`,
erase it and expand the matching YASnippet from `+file-templates-dir`."
    (when (and buffer-file-name
               (string-match-p "\\.rb\\'" buffer-file-name)    ; only .rb
               (not (file-exists-p buffer-file-name))          ; brand‑new file
               (= (buffer-size) 0))                            ; buffer is empty
      (let* ((tpl-dir    +file-templates-dir)
             (fname      (file-name-nondirectory buffer-file-name))
             (snippet    (cond
                          ((string-match-p "_spec\\.rb\\'" fname)
                           "__ruby-spec")
                          ((string-match-p "signup_controller\\.rb\\'" fname)
                           "__signup-controller")
                          ((string-match-p "sigin_controller\\.rb\\'" fname)
                           "__signin-controller")
                          ((string-match-p "refresh_controller\\.rb\\'" fname)
                           "__refresh-controller")
                          ((string-match-p "_controller\\.rb\\'" fname)
                           "__ruby-controller"))))
        (when snippet
          (erase-buffer)
          (yas-expand-snippet
           (with-temp-buffer
             (insert-file-contents (expand-file-name snippet tpl-dir))
             (buffer-string)))))))

  ;; Run our inserter *after* yasnippet is ready, on every find-file
  (add-hook 'find-file-hook #'kyo/auto‑insert‑ruby‑template))
