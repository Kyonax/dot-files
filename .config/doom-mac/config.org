...
(after! flycheck
  ...
  (flycheck-define-checker eslint-d-strict
    ...
    :command ("eslint_d_strict" "--format" "unix" source-inplace)
...
(defun kyo/eslint-strict-fix-file ()
  ...
  (let* ((project-root
          (or (locate-dominating-file buffer-file-name "eslint.config.mjs")
              (locate-dominating-file buffer-file-name "package.json")
              (user-error "Not inside an ESLint project")))
         (default-directory project-root)
         (file (file-relative-name buffer-file-name project-root))
         (quoted (shell-quote-argument file))
         (cmd (format "eslint --fix --debug --no-ignore %s" quoted)))
    (message "Running: %s" cmd)
    (let ((output (shell-command-to-string cmd)))
      (when (and output (not (string-empty-p output)))
        (message "%s" output)))
    (revert-buffer :ignore-auto :noconfirm)
    (message "eslint --fix --debug applied and buffer reloaded.")))
...
(setq my-gptel-directives
      `((default . ,(with-temp-buffer
                (insert-file-contents "~/.config/doom/gptel-directives/default.md")
                (buffer-string)))
        (studying . ,(with-temp-buffer
                (insert-file-contents "~/.config/doom/gptel-directives/studying.md")
                (buffer-string)))
        (ambiguous-variables-vue . ,(with-temp-buffer
                (insert-file-contents "~/.config/doom/gptel-directives/ambiguous-variables-vue.md")
                (buffer-string)))
        (code-style-vue . ,(with-temp-buffer
                (insert-file-contents "~/.config/doom/gptel-directives/code-style-vue.md")
                (buffer-string)))
        (documentation . ,(with-temp-buffer
                (insert-file-contents "~/.config/doom/gptel-directives/documentation.md")
                (buffer-string)))))
#+end_src
...
Shows only first line of each directive. Writes response into a new .md buffer."
  ...
  (let* ((cand-list
    ...
    (when selected-key
      (let* ((directive-text (cdr (assoc selected-key my-gptel-directives)))
...
#+end_src
*** MULTIPLE CURSORs
Configure the ~multiple-cursors~ package keybindings.
| Keybinding  | Function                   | Description                         |
|-------------+----------------------------+-------------------------------------|
| C-S-c C-S-c | mc/edit-lines              | Edit all lines in a selected region |
| C->         | mc/mark-next-like-this     | Mark the next occurrence            |
| C-<         | mc/mark-previous-like-this | Mark the previous occurrence        |
| C-c C-<     | mc/mark-all-like-this      | Mark all occurrences in the buffer  |
