...
#+begin_src emacs-lisp :tangle no
  ;; Define custom plinkx method to simplify the connection
               ...
               `("plinkw"
                ...
                (tramp-login-args (("-ssh")
				   (,(format
				    "dockware@127.0.0.1 -pw dockware"))))
                (tramp-remote-shell "/bin/sh")))
  (setq tramp-default-method "simplified")
...
(add-to-list 'tramp-methods
        '("plink-simple"
        (tramp-login-program tramp-plink-program)
        (tramp-login-args (("-load" "%h")))
        (tramp-remote-shell "/bin/sh")
        (tramp-remote-shell-args ("-i"))))

(when (eq system-type 'windows-nt)
  ;; Define your PuTTY directory path (update this to your actual path)
  (defvar putty-directory "C:/Users/MORENOC0/scoop/apps/putty/current")

  ;; Ensure PuTTY directory is added to PATH and exec-path
  (when (file-directory-p putty-directory)
    (unless (member putty-directory (split-string (getenv "PATH") ";"))
      (setenv "PATH" (concat putty-directory ";" (getenv "PATH")))
      (add-to-list 'exec-path putty-directory)))

  ;; Set tramp configurations
  (setq tramp-verbose 3
        tramp-plink-program (expand-file-name "plink.exe" putty-directory))

  ;; Define a custom TRAMP method for simplified plink usage
  (add-to-list 'tramp-methods
               '("plink-simple"
                 (tramp-login-program tramp-plink-program)
                 (tramp-login-args (("-load" "%h")))
  ...
  ;; Set the custom method as the default for TRAMP
  (setq tramp-default-method "plink-simple")

  ;; Ensure better timeout handling for TRAMP connections
