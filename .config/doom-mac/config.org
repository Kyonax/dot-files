...
(after! flycheck
  (setq flycheck-checker-error-threshold 2000)
  (flycheck-define-checker eslint
    "Run eslint (a strict eslint_d wrapper)."
    :command ("eslint" "--format" "unix" source-inplace)
    :standard-input (eval (not (buffer-file-name)))
    :error-patterns ((error line-start (file-name) ":" line ":" column ": "
                            (message (one-or-more not-newline)) " [Error" (* not-newline) "]" line-end)
                     (warning line-start (file-name) ":" line ":" column ": "
                              (message (one-or-more not-newline)) " [Warning" (* not-newline) "]" line-end))
  ...
  (add-to-list 'flycheck-checkers 'eslint)
  (setq-default flycheck-disabled-checkers
                '(javascript-eslint javascript-jshint javascript-standard typescript-tslint vue-eslint))
  (add-hook 'flycheck-mode-hook (lambda ()
                                  (when (derived-mode-p 'js-mode 'js2-mode 'typescript-mode 'web-mode)
                                    (flycheck-select-checker 'eslint)))))
#+end_src
